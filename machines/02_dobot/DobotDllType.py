...
def GetQueuedCmdCurrentIndex(api):
    ...
    if masterDevType == DevType.Conntroller and slaveDevType == DevType.MagicianLite:
        ...
        while(True):
            result = api.GetQueuedCmdCurrentIndex( c_int(-1), byref(queuedCmdIndex1))
...
def SetQueuedCmdStartExec(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        while (True):
            result = api.SetQueuedCmdStartExec( c_int(-1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetQueuedCmdStopExec(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        while (True):
            result = api.SetQueuedCmdStopExec( c_int(-1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetQueuedCmdForceStopExec(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        while (True):
            result = api.SetQueuedCmdForceStopExec( c_int(-1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetQueuedCmdClear(api):
    # 滑轨特殊处理
    # return [api.SetQueuedCmdClear( c_int(slaveId))]
    if slaveDevType == DevType.Magician:
        while(True):
            result = api.SetQueuedCmdClear( )
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        while (True):
            result = api.SetQueuedCmdClear( c_int(-1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def GetDeviceVersion(api):
    ...
    if (masterDevType == DevType.Conntroller and (slaveDevType == DevType.MagicianLite or slaveDevType == DevType.Idle)):
        while(True):
            result = api.GetDeviceVersion( c_int(-1), byref(deviceVersion))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        while(True):
            result = api.GetDeviceVersion(  byref(deviceVersion))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetDeviceWithL(api, isWithL, version=0, isQueued=0):
    ...
    while(True):
        print(tempSlaveId)
        result = api.SetDeviceWithL( c_int(tempSlaveId), c_bool(isWithL), c_uint8(version), c_bool(isQueued), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceWithL(api):
    ...
    while(True):
        result = api.GetDeviceWithL( c_int(tempSlaveId), byref(isWithL))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceTime(api):
    time = c_uint32(0)
    while(True):
        result = api.GetDeviceTime(  byref(time))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceID(api):
    ...
    timeout = False
    while(True):
        result = api.GetDeviceID( c_int(-1), byref(deviceID))
        if result != DobotCommunicate.DobotCommunicate_NoError:
...
def GetDeviceInfo(api):
    ...
    while(True):
        result = api.GetDeviceInfo(  byref(info))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetPoseL(api):
    ...
    l = c_float(0)
    while(True):
        result = api.GetPoseL( c_int(tempSlaveId), byref(l))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetHOMECmd(api, temp, isQueued=0):
    ...
    if masterDevType == DevType.Magician:
        ...
        while(True):
            result = api.SetHOMECmd( c_int(-1), byref(cmd), isQueued, byref(queuedCmdIndex1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        # 其他情况
        while(True):
            result = api.SetHOMECmd( c_int(slaveDevType), byref(cmd), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetJOGLParams(api, velocity, acceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetJOGLParams( c_int(tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetJOGLParams(api):
    ...
    param = JOGLParams()
    while(True):
        result = api.GetJOGLParams( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetJOGCommonParams(api, value_velocityratio, value_accelerationratio, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        while(True):
            result = api.SetJOGCommonParams( c_int(-1), byref(param), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetJOGCmd(api, isJoint, cmd, isQueued=0):
    ...
    if cmd == 0:
        while(True):
            result = api.SetJOGCmd( c_int(-1), byref(cmdParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        while(True):
            result = api.SetJOGCmd( c_int(tempSlaveId), byref(cmdParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetPTPLParams(api, velocity, acceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetPTPLParams( c_int(tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetPTPLParams(api):
    ...
    param = PTPLParams()
    while(True):
        result = api.GetPTPLParams( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetPTPCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        while(True):
            result = api.SetPTPCommonParams( c_int(-1), byref(pbParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetPTPWithLCmd(api, ptpMode, x, y, z, rHead, l, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        queuedCmdIndex1 = c_uint64(0)
        while(True):
            result = api.SetPTPWithLCmd( c_int(-1), byref(cmd), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(2)
                continue
            break
        while(True):
            result = api.SetPTPCmd(  byref(cmd1), isQueued, byref(queuedCmdIndex1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(2)
                continue
            break
...
def SetCPRHoldEnable(api, isEnable):
    while(True):
        result = api.SetCPRHoldEnable(  c_bool(isEnable))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetCPRHoldEnable(api):
    ...
    while(True):
        result = api.GetCPRHoldEnable(  byref(isEnable))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetCP2Cmd(api, cpMode, x, y, z, isQueued=0):
    ...
    while(True):
        result = api.SetCP2Cmd(  byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(2)
            continue
        break
...
def SetCPCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetCPCommonParams(  byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetCPCommonParams(api):
    pbParam = CPCommonParams()
    while(True):
        result = api.GetCPCommonParams(  byref(pbParam ))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetCircleCmd(api, cirPoint, toPoint,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetCircleCmd(  byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetARCCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetARCCommonParams(  byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetARCCommonParams(api):
    pbParam = ARCCommonParams()
    while(True):
        result = api.GetARCCommonParams(  byref(pbParam ))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIOMultiplexing(api, address, multiplex, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetIOMultiplexing( c_int(tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOMultiplexing(api,  addr):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetIOMultiplexing( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIODO(api, address, level, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetIODO( c_int(tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIODO(api,  addr):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetIODO( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIOPWM(api, address, frequency, dutyCycle,  isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetIOPWM( c_int(tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOPWM(api,  addr):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetIOPWM( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIODI(api, addr):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetIODI( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEMotor(api, index, isEnabled, speed,  isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetEMotor( c_int(tempSlaveId), byref(emotor), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEMotorS(api, index, isEnabled, speed, distance,  isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetEMotorS( c_int(tempSlaveId), byref(emotorS), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOADC(api, addr):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetIOADC( c_int(tempSlaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetColorSensor(api, isEnable, colorPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetColorSensor( c_int(tempSlaveId), enable, port, version, 1, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetColorSensor(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetColorSensor( c_int(tempSlaveId), byref(r),  byref(g),  byref(b))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetInfraredSensor(api,  isEnable, infraredPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetInfraredSensor( c_int(tempSlaveId), enable, port, version, 1, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetInfraredSensor(api, infraredPort):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetInfraredSensor( c_int(tempSlaveId), port,  byref(value))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLostStepParams(api, threshold, isQueued=0):
    ...
    t = c_float(threshold)
    while(True):
        result = api.SetLostStepParams(  t, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetUART4PeripheralsType(api):
    ...
    if (masterDevType == DevType.Conntroller and slaveDevType == DevType.MagicianLite) or (masterDevType == DevType.Conntroller and slaveDevType == DevType.Idle):
        while(True):
            result = api.GetUART4PeripheralsType( c_int(-1), byref(type))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break 
        ...
        while(True):
            result = api.GetUART4PeripheralsType(  byref(type))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def GetDeviceVersionEx(api):       #2019.6.25 song 控制盒+Magician Lite时，获取控制盒的版本
    ...
    if masterDevType == DevType.Conntroller and slaveDevType == DevType.MagicianLite:
        ...
        while(True):
            result = api.GetDeviceVersion( c_int(-1), byref(deviceVersion1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        while(True):
            result = api.GetDeviceVersion(  byref(deviceVersion2))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetPTPWithLCmdEx(api, ptpMode, x, y, z, rHead,  l, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        while(True):
            ...
            break
        while(True):
            result = api.GetQueuedCmdCurrentIndex(  byref(queuedCmdIndex1))
...
def SetUpgradeFWReadyCmd(api,fwSize, md5):
    ...
    # # 只发送给主设备
    # result = api.SetUpgradeFWReadyCmd( c_int(-1), byref(upgradeFWReadyCmd))
    # return result
    ...
    if masterDevType == DevType.Conntroller and (slaveDevType == DevType.MagicianLite or slaveDevType == DevType.Idle):
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetUpgradeFWReadyCmd( c_int(tempSlaveId), byref(upgradeFWReadyCmd))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetUpgradeFWReadyCmd(api,fwSize, md5):
    ...
    if masterDevType == DevType.Conntroller and (slaveDevType == DevType.MagicianLite or slaveDevType == DevType.Idle):
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetUpgradeFWReadyCmd( c_int(tempSlaveId), byref(upgradeFWReadyCmd), byref(isUpgrade))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetMotorMode(api, mode):
    while(True):
        result = api.SetMotorMode(  c_int(mode))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break 
...
def GetMotorMode(api):
    mode = c_int(0)
    while(True):
        result = api.GetMotorMode(  byref(mode))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break 
...
def SetIOMultiplexingExt(api, address, multiplex, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetIOMultiplexing( c_int(-1), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOMultiplexingExt(api, addr):
    ...
    param.address = addr
    while(True):
        result = api.GetIOMultiplexing( c_int(-1), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOADCExt(api, addr):
    ...
    param.address = addr
    while(True):
        result = api.GetIOADC( c_int(-1), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIOPWMExt(api, address, frequency, dutyCycle,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetIOPWM( c_int(-1), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOPWMExt(api, addr):
    ...
    param.address = addr
    while(True):
        result = api.GetIOPWM( c_int(-1), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIODIExt(api, addr):
    ...
    param.address = addr
    while(True):
        result = api.GetIODI( c_int(-1), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIODOExt(api, address, level, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetIODO( c_int(-1), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIODOExt(api, addr):
    ...
    param.address = addr
    while(True):
        result = api.GetIODO( c_int(-1), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEMotorExt(api, index, isEnabled, speed, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEMotor( c_int(-1), byref(emotor), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEMotorSExt(api, index, isEnabled, speed, distance, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEMotorS( c_int(-1), byref(emotorS), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetColorSensorExt(api, isEnable, colorPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetColorSensor( c_int(-1), enable, port, version, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetInfraredSensorExt(api,  isEnable, infraredPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetInfraredSensor( c_int(-1), enable, port, version, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetInfraredSensorExt(api, infraredPort):
    ...
    while(True):
        result = api.GetInfraredSensor( c_int(-1), port,  byref(value))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetColorSensorExt(api, index):
    ...
    b = c_ubyte(0)
    while(True):
        result = api.GetColorSensor( c_int(-1), byref(r),  byref(g),  byref(b))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedColorSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedColorSensor( c_int(tempSlaveId), byref(r),  byref(g),  byref(b), byref(Cct))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedColorSensorExt(api, SeeedPort,isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedColorSensor( c_int(tempSlaveId), port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedDistanceSensorExt(api, SeeedPort):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedDistanceSensor( c_int(tempSlaveId), port, byref(distance))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedTempSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedTempSensor( c_int(tempSlaveId), port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedTempSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedTempSensor( c_int(tempSlaveId), byref(tem),  byref(hum))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedLightSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedLightSensor( c_int(tempSlaveId), port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedLightSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedLightSensor( c_int(tempSlaveId), byref(lux))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedRgbExt(api, SeeedPort, Rgb, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedRgb( c_int(tempSlaveId), port, rgb, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def RestartMagicBox(api):
    while(True):
        result = api.RestartMagicBox( c_int(-1))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLostStepEnableAndParamsCmd(api, enable, threshlod, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetLostStepEnableAndParamsCmd(  c_uint8(enable), c_float(threshlod), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetLostStepEnableAndParamsCmd(api):
    ...
    threshlod = c_float(0)
    while(True):
        result = api.GetLostStepEnableAndParamsCmd(  byref(enable), byref(threshlod))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEndEffectorType(api, endType=0, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetEndEffectorType(  isQueued, c_uint8(endType), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break  
...
def GetEndEffectorType(api):
    endType = c_uint8(0)
    while(True):
        result = api.GetEndEffectorType(  byref(endType))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetServoAngle(api, servoId, angle, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetServoAngle( c_int(-1), isQueued, c_uint8(servoId), c_float(angle), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break 
...
def GetServoAngle(api, servoId):
    angle = c_float(0)
    while(True):
        result = api.GetServoAngle( c_int(-1),  c_uint8(servoId) ,byref(angle))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetArmSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetArmSpeedRatio(  isQueued, c_uint8(paramsMode), c_uint8(speedRatio),  byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break 
...
def GetArmSpeedRatio(api, paramsMode=0):
    ...
    while(True):
        result = api.GetArmSpeedRatio(   c_uint8(paramsMode), byref(speedRatio))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetLSpeedRatio( c_int(-1), isQueued, c_uint8(paramsMode), c_uint8(speedRatio), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetLSpeedRatio(api, paramsMode):
    speedRatio = c_uint8(0)
    while(True):
        result = api.GetLSpeedRatio( c_int(-1), c_uint8(paramsMode), byref(speedRatio))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def PrintInfo(api, info):
    ...
    while(True):
        result = api.PrintInfo( c_int(-1), szPara)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetProgbar(api, progbar):
    while(True):
        result = api.SetProgbar( c_int(-1), c_uint8(progbar))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
