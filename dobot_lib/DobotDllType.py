...
import os
from os.path import join as path_joiner
...
def load(dll_path=None):
    if platform.system() == "Windows":
        print("The dll you are using is 64-bit. In order to run smoothly, please ensure that your python environment"
              " is also 64-bit.")
        print("The python environment is：", platform.architecture())
        if dll_path is None:
            return CDLL("./DobotDll.dll", RTLD_GLOBAL)
        else:
            return CDLL(path_joiner(dll_path, "DobotDll.dll"), RTLD_GLOBAL)
    ...
    try:
        if masterDevType == DevType.Conntroller:
            if connectInfo.slaveDevInfo1.type == 0 and connectInfo.slaveDevInfo2.type == 0:
                ...
                fwName = str(connectInfo.slaveDevInfo1.firmwareName, encoding="utf-8").strip(
                    b'\x00'.decode()) if connectInfo.slaveDevInfo1.type != DevType.Idle else str(
                    connectInfo.slaveDevInfo2.firmwareName, encoding="utf-8").strip(b'\x00'.decode())
                fwVer = str(connectInfo.slaveDevInfo1.firwareVersion, encoding="utf-8").strip(
                    b'\x00'.decode()) if connectInfo.slaveDevInfo1.type != DevType.Idle else str(
                    connectInfo.slaveDevInfo2.firwareVersion, encoding="utf-8").strip(b'\x00'.decode())
    ...
    if (masterDevType == DevType.Conntroller and (
        ...
        for _ in set_timeout():
            ...
            if result != DobotCommunicate.DobotCommunicate_NoError:
                ...
                continue
            break
        return [deviceVersion.fw_majorVersion, deviceVersion.fw_minorVersion, deviceVersion.fw_revision,
                deviceVersion.fw_alphaVersion,
                deviceVersion.hw_majorVersion, deviceVersion.hw_minorVersion, deviceVersion.hw_revision,
...
def SetDeviceWithL(api, isWithL, version=0, isQueued=0):
    ...
    for _ in set_timeout():
        print(tempSlaveId)
        result = api.SetDeviceWithL(c_int(masterId), c_int(tempSlaveId), c_bool(isWithL), c_uint8(version),
                                    c_bool(isQueued), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetPose(api, timeout=2):
    pose = Pose()
    for _ in set_timeout(timeout):
...
def SetEndEffectorParams(api, xBias, yBias, zBias, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetEndEffectorParams(c_int(masterId), c_int(slaveId), byref(param), isQueued,
...
def SetEndEffectorLaser(api, enableCtrl, on, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetEndEffectorLaser(c_int(masterId), c_int(slaveId), enableCtrl, on, isQueued,
                                         byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEndEffectorSuctionCup(api, enableCtrl, on, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetEndEffectorSuctionCup(c_int(masterId), c_int(slaveId), enableCtrl, on, isQueued,
                                              byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEndEffectorGripper(api, enableCtrl, on, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetEndEffectorGripper(c_int(masterId), c_int(slaveId), enableCtrl, on, isQueued,
                                           byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetJOGJointParams(api, j1Velocity, j1Acceleration, j2Velocity, j2Acceleration, j3Velocity, j3Acceleration,
    ...
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetJOGJointParams(c_int(masterId), c_int(slaveId), byref(jogParam), isQueued,
                                       byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetJOGCoordinateParams(api, xVelocity, xAcceleration, yVelocity, yAcceleration, zVelocity, zAcceleration, rVelocity,
    ...
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetJOGCoordinateParams(c_int(masterId), c_int(slaveId), byref(param), isQueued,
                                            byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetJOGCommonParams(api, value_velocityratio, value_accelerationratio, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        for _ in set_timeout():
            result = api.SetJOGCommonParams(c_int(masterId), c_int(slaveId), byref(param), isQueued,
                                            byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
...
def SetJOGCmd(api, isJoint, cmd, isQueued=0):
    ...
    if cmd == 0:
        ...
        for _ in set_timeout():
            result = api.SetJOGCmd(c_int(masterId), c_int(tempSlaveId), byref(cmdParam), isQueued,
                                   byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
...
def SetPTPCoordinateParams(api, xyzVelocity, xyzAcceleration, rVelocity, rAcceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetPTPCoordinateParams(c_int(masterId), c_int(slaveId), byref(pbParam), isQueued,
                                            byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetPTPCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        for _ in set_timeout():
            result = api.SetPTPCommonParams(c_int(masterId), c_int(slaveId), byref(pbParam), isQueued,
                                            byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
...
def SetPTPCmd(api, ptpMode, x, y, z, rHead, isQueued=0, timeout=2):
    cmd = PTPCmd()
    cmd.ptpMode = ptpMode
    cmd.x = x
    cmd.y = y
...
def SetARCCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetARCCommonParams(c_int(masterId), c_int(slaveId), byref(pbParam), isQueued,
                                        byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetIOMultiplexing(api, address, multiplex, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    for _ in set_timeout():
        result = api.SetIOMultiplexing(c_int(masterId), c_int(tempSlaveId), byref(param), isQueued,
                                       byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetAngleSensorStaticError(api, rearArmAngleError, frontArmAngleError):
    ...
    for _ in set_timeout():
        result = api.SetAngleSensorStaticError(c_int(masterId), c_int(slaveId), c_rearArmAngleError,
                                               c_frontArmAngleError)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetAngleSensorStaticError(api):
    ...
    for _ in set_timeout():
        result = api.GetAngleSensorStaticError(c_int(masterId), c_int(slaveId), byref(rearArmAngleError),
...
def GetAngleSensorCoef(api):
    ...
    for _ in set_timeout():
        result = api.GetAngleSensorCoef(c_int(masterId), c_int(slaveId), byref(rearArmAngleCoef),
...
def SetColorSensor(api, isEnable, colorPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    for _ in set_timeout():
        result = api.SetColorSensor(c_int(masterId), c_int(tempSlaveId), enable, port, version, 1,
                                    byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetInfraredSensor(api, isEnable, infraredPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    for _ in set_timeout():
        result = api.SetInfraredSensor(c_int(masterId), c_int(tempSlaveId), enable, port, version, 1,
                                       byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetUART4PeripheralsType(api):
    type = c_uint8(0)
    if (masterDevType == DevType.Conntroller and slaveDevType == DevType.MagicianLite) or (
            masterDevType == DevType.Conntroller and slaveDevType == DevType.Idle):
        for _ in set_timeout():
    ...
    if masterDevType == DevType.Conntroller and slaveDevType == DevType.MagicianLite:
        ...
        for _ in set_timeout():
            ...
            if result != DobotCommunicate.DobotCommunicate_NoError:
                ...
                continue
            break
        list_MagicBoxVersion = [deviceVersion1.fw_majorVersion, deviceVersion1.fw_minorVersion,
                                deviceVersion1.fw_revision, deviceVersion1.fw_alphaVersion,
                                deviceVersion1.hw_majorVersion, deviceVersion1.hw_minorVersion,
                                deviceVersion1.hw_revision, deviceVersion1.hw_alphaVersion]
        for _ in set_timeout():
            ...
            if result != DobotCommunicate.DobotCommunicate_NoError:
                ...
                continue
            break
        list_MagicianLiteVersion = [deviceVersion2.fw_majorVersion, deviceVersion2.fw_minorVersion,
                                    deviceVersion2.fw_revision, deviceVersion2.fw_alphaVersion,
                                    deviceVersion2.hw_majorVersion, deviceVersion2.hw_minorVersion,
                                    deviceVersion2.hw_revision, deviceVersion2.hw_alphaVersion]
...
def SetEndEffectorSuctionCupEx(api, enableCtrl, on, isQueued=0, timeout=2):
...
def GetUpgradeFWReadyCmd(api, fwSize, md5):
    ...
    if masterDevType == DevType.Conntroller and (slaveDevType == DevType.MagicianLite or slaveDevType == DevType.Idle):
        ...
        tempSlaveId = slaveId
    for _ in set_timeout():
        result = api.GetUpgradeFWReadyCmd(c_int(masterId), c_int(tempSlaveId), byref(upgradeFWReadyCmd),
                                          byref(isUpgrade))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetInfraredSensorExt(api, isEnable, infraredPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetInfraredSensor(c_int(masterId), c_int(-1), enable, port, version, isQueued,
                                       byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetLostStepEnableAndParamsCmd(api, enable, threshlod, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetLostStepEnableAndParamsCmd(c_int(masterId), c_int(slaveId), c_uint8(enable), c_float(threshlod),
                                                   isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEndEffectorType(api, endType=0, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetEndEffectorType(c_int(masterId), c_int(slaveId), isQueued, c_uint8(endType),
                                        byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetServoAngle(api, servoId, angle, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetServoAngle(c_int(masterId), c_int(-1), isQueued, c_uint8(servoId), c_float(angle),
                                   byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetArmSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetArmSpeedRatio(c_int(masterId), c_int(slaveId), isQueued, c_uint8(paramsMode),
                                      c_uint8(speedRatio), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    for _ in set_timeout():
        result = api.SetLSpeedRatio(c_int(masterId), c_int(-1), isQueued, c_uint8(paramsMode), c_uint8(speedRatio),
                                    byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
